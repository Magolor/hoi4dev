...
目前我们需要以GitHub Repo的形式安装，后续我们将把HOI4DEV升级为Python库。请寻找一个不会频繁移动改名的电脑位置，运行命令：
...
如果正确安装，在命令行中会有输出信息`hoi4dev version 0.1.0.3 successfully installed!`或根据当前的HOI4DEV版本号输出类似信息。
