...
def is_ccl(path:str):
    ...
    if not (ext in SUPPORTED_TEXTUAL_RESOURCES_SUFFIXES):
        raise ValueError(f"Unsupported file type: {ext}!")
    return (ext in SUPPORTED_CCL_RESOURCES_SUFFIXES)

def is_json(path:str):
    return get_file_ext(path) == 'json'
...
def tokenize_ccl_string(ccl_string):
    ...
    sentences = mark_ccl_string(ccl_string)
    tokens = []
...
def ccl_token_type(t):
    if t == '§':
        return 'BUG' # There is one bug in HoI4 game file in `common/units/equipment/ship_hull_carrier.txt` that contains miss-typed `§`.
    ...
    if t == '}':
        return 'END'
    if t.strip() == 'rgb' or t.strip() == 'hsv': # Only support lowercase since `RGB` could be a country tag (after the Gotterdammerung DLC)
        return 'RGB'
...
def deduce_ext_by_ccl_content(content):
    content = content.lower()
    if 'objecttypes =' in content: return 'gfx'
    if 'spritetypes =' in content: return 'gfx'
    if 'guitypes =' in content: return 'gui'
    if 'containerwindowtype =' in content: return 'gui'
    if 'pdxmesh =' in content: return 'asset'
    if 'entity =' in content: return 'asset'
    if 'sound =' in content: return 'asset'
    if 'soundeffects =' in content: return 'asset'
    if 'compressor =' in content: return 'asset'
    if 'falloff =' in content: return 'asset'
    if 'steam_id =' in content: return 'dlc'
    return 'txt'

def load_ccl(path):
    return load_txt(path)

def save_ccl(content, path, deduce_ext:bool = True):
    return save_txt(content, get_file_name(path, ext=(deduce_ext_by_ccl_content(content) if deduce_ext else None)))
...
def CCL2List(ccl_string):
    ...
    while i < len(tokens):
        token = tokens[i]
        if ccl_token_type(token) == 'BUG':
            i += 1; continue
        if ccl_token_type(token) == 'RGB':
            assert (i+1<len(tokens) and ccl_token_type(tokens[i+1])=='START'), "Invalid `RGB` type!"
            j = i+1
            while ccl_token_type(tokens[j])!='END':
                j += 1
            value = ' '.join([token.strip().lower()] + tokens[i+1:j+1])
            i = j+1
            op = stack.pop()
            key = ccl_eval(stack.pop())
            assert (ccl_token_type(op)=='EQUAL'), "Only assignment can have an rgb as right value!"
            stack[-1].append({key: value})
            continue
        if stack[-1] in ['=', '<', '>']:
            op = stack.pop()
            key = ccl_eval(stack.pop())
            if ccl_token_type(token) == 'START':
                assert (op == '='), "Only assignment can have a dictionary as right value!"
                stack[-1].append({key: list()})
                stack.append(stack[-1][-1][key])
            else:
                if ccl_token_type(op)=='EQUAL':
                    stack[-1].append({key: ccl_eval(token)})
                else:
                    stack[-1].append({' '.join([key, op, token]): None})
        elif ccl_token_type(token) == 'END':
            l = list()
            while not isinstance(stack[-1], list):
                l.append(ccl_eval(stack.pop()))
            stack[-1].extend(l[::-1])
            stack.pop()
        elif ccl_token_type(token) == 'START':
            stack[-1].append(list())
...
def ccl_convert(src_path, tgt_path):
    '''
    ...
    Return:
        str. tgt_path.
    '''
    assert (is_ccl(src_path) or is_json(src_path)), "Unsupported file type for source file! Only CCL and json dict are supported!"
    deduce_ext = False
    if get_file_ext(tgt_path) == '':
        if is_json(src_path):
            tgt_path = get_file_name(tgt_path, ext='txt')
            deduce_ext = True
        else:
            tgt_path = get_file_name(tgt_path, ext='json')
    touch_file(tgt_path)
    if (not is_json(src_path)) and (not is_json(tgt_path)):
        copy_file(src_path, tgt_path); return tgt_path
    if (is_json(src_path)) and (is_json(tgt_path)):
        save_json(load_json(src_path), tgt_path, indent=4); return tgt_path
    if (not is_json(src_path)) and (is_json(tgt_path)):
        save_json(CCL2Dict(load_ccl(src_path)), tgt_path, indent=4); return tgt_path
    if (is_json(src_path)) and (not is_json(tgt_path)):
        save_ccl(Dict2CCL(load_json(src_path)), tgt_path, deduce_ext=deduce_ext,); return tgt_path

def ccl_convert_batch(src_path, tgt_path, ext=False):
    '''
    ...
    Args:
        ...
        ext: str. Extension of the target file. Default is False, which means it is different from the source file.
    Return:
        List[str]. List of converted file paths.
    '''
    return [ccl_convert(pj(src_path, f), get_file_name(pj(tgt_path, f), ext=ext)) for f in list_files(src_path)]
